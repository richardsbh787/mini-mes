...
from app.services.work_order_routing_step_completion import validate_work_order_execution_completion_ready_routing_step
...
class WorkOrderRoutingStepCompletionTests(unittest.TestCase):
    ...
    def _create_work_order_with_snapshot(
        ...
        *,
        ...
        sales_order = _add_sales_order(db, f"SO-COMP-{suffix}")
        product = _add_product(db, f"FG-COMP-{suffix}")
        line = _add_line(db, f"LINE-COMP-{suffix}")
        routing = _add_routing(db, f"FG-COMP-{suffix}")
        ...
        work_order = create_work_order_record(
            ...
            work_order=WorkOrderCreate(
                work_order_no=f"WO-COMP-{suffix}",
    ...
    def test_same_step_completion_readiness_is_allowed(self) -> None:
        db = self._new_db()
        work_order_id = self._create_work_order_with_snapshot(db, "1")

        completion = validate_work_order_execution_completion_ready_routing_step(
            db=db,
        ...
        self.assertEqual(completion.current_step.seq_no, 20)
        self.assertEqual(completion.target_step.seq_no, 20)
        self.assertEqual(completion.completion_step.seq_no, 20)
        self.assertEqual(completion.snapshot_id, list_work_order_reads(db=db)[0].routing_snapshot.snapshot_id)

    def test_forward_target_completion_readiness_is_allowed(self) -> None:
        db = self._new_db()
        ...
        self.assertEqual(
            (completion.current_step.seq_no, completion.target_step.seq_no, completion.completion_step.seq_no),
            (10, 20, 20),
        )
    ...
    def test_completion_target_mismatch_is_rejected(self) -> None:
        db = self._new_db()
    ...
    def test_missing_snapshot_remains_rejected_through_completion_path(self) -> None:
        db = self._new_db()
    ...
    def test_missing_work_order_remains_rejected_through_completion_path(self) -> None:
        db = self._new_db()
